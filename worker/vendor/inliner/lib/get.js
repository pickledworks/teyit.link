...
module.exports = function get(url, options) {
  ...
  var requestAdaptor = request;

  if (inliner.options.adaptor !== undefined) {
    requestAdaptor = inliner.options.adaptor;
  }
  
  cache[url] = new Promise(function (resolve) {
    requestAdaptor(encodeURI(url), settings, function response(error, res, body) {
      if (error) {
